...
# --------------------------------------------------------------------------
# 経路の書き直し（route_follower.py）。値は実機（sorot_spike の
# RoughSpot/Python/Common/config.py）と同じ。
#
# **シミュレータはこの置き場を使わない。** ブラウザにファイルシステムが無く、
# 自己位置は JS から渡し、書き直した行も JS へ返す（sim/pyscript/route_follow_bridge.py）。
# 読み込みで落ちないよう、実機と同じ値をそのまま持たせてある。
# --------------------------------------------------------------------------
#: 走行中のシナリオ差し替え。中身は custom.json と同じ形のコマンドセットの配列
COMM_SCN_FILE = COMM_DIR + "/sorot_scenario_update.json"
COMM_SCN_TMP = COMM_DIR + "/sorot_scenario_update.tmp"

#: ETラリーの経路。Python が置き、C++ が1回だけ引き取る（引き取ると消える）
COMM_ROUTE_FILE = COMM_DIR + "/sorot_route.json"
COMM_ROUTE_TMP = COMM_DIR + "/sorot_route.tmp"

#: 走行中の自己位置。C++ が 100ms ごとに置く
COMM_POSE_FILE = COMM_DIR + "/sorot_pose.json"

#: 自己位置がこれより古ければ使わない。3周期ぶん待っても来なければ C++ が止まっている
POSE_STALE_SEC = 0.3
